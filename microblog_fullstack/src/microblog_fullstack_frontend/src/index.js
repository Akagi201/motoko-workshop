import { microblog_fullstack_backend } from "../../declarations/microblog_fullstack_backend";
...
async function post() {
  ...
  try {
    await microblog_fullstack_backend.post(textarea.value, otp);
    textarea.value = "";
  } catch (err) {
    console.log(err);
...
async function set_name() {
  ...
  let otp = document.getElementById("otp").value;
  try {
    await microblog_fullstack_backend.set_name(textarea.value, otp);
  } catch (err) {
    console.log(err);
...
async function follow() {
  ...
  try {
    await microblog_fullstack_backend.follow(
...
async function reflesh_author_list() {
  let followed = await microblog_fullstack_backend.follows();
...
async function load_posts(author) {
  ...
  switch (author) {
    case "0":
      posts = await microblog_fullstack_backend.posts(1000);
      break;
    case "1":
      posts = await microblog_fullstack_backend.timeline([], 1000);
      break;
    default:
      posts = await microblog_fullstack_backend.timeline(
        [Principal.fromText(author)],
        1000
      );
  }
  console.log(posts);
  ...
  for (var i = 0; i < posts.length; i++) {
    ...
    post.innerText =
      "[" +
      new Date(Number(posts[i].time / 1000000n)).toLocaleString() +
      "] <" +
      posts[i].author +
      "> " +
      posts[i].text;
